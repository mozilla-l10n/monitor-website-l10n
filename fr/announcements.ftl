...
announcement-free-data-breach-monitoring-title =
    { $emailAddressesCount ->
        [one] Surveillez gratuitement les fuites de données pour un maximum de { NUMBER($emailAddressesCount) } adresse e-mail.
       *[other] Surveillez gratuitement les fuites de données pour un maximum de { NUMBER($emailAddressesCount) } adresses e-mail.
    }
